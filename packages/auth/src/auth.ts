...
export function AuthFunc(options: AuthOptions,
  axios: AxiosInstance) {
  const storage: AuthStorage | AuthStorageAsync = useStorage(options.storage.driver)
  ...
  async function login<P = AxiosRequestConfig>(payload: P, callback: ICallBackUser) {
    callback?.onLoading?.(true)
    try {
      delete axios.defaults.headers.common.Authorization
      const res = await axios.request(
        merge(options.endpoints.login, payload),
      )
  ...
  async function forgotPassword<P = AxiosRequestConfig>(payload: P, callback: ICallBack) {
    callback?.onLoading?.(true)
    try {
      const res = await axios.request(
  ...
  async function signup<P = AxiosRequestConfig>(payload: P, callback: ICallBackUser) {
    callback?.onLoading?.(true)
    try {
      const res = await axios.request(
        merge(options.endpoints.login, payload),
  ...
  async function isExpired(callback?: ICallBack) {
    callback?.onLoading?.(true)
    const expireTime = await storage.get<number>(
      options.expiredStorage,
    )
  ...
  async function getRefreshToken() {
    return await storage.get<string>(
      options.refreshToken.storageName,
    )
  }
  ...
  async function refreshToken(callback?: ICallBack) {
    ...
    try {
      ...
      const tokenData = get(
        res.data,
        options.refreshToken.accessTokenProperty,
      )
